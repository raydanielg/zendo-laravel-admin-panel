                    ...
                    <div class="card card-body h-100">
                        ...
                        <div class="p-lg-4 p-3 rounded bg-F6F6F6">
                            ...
                            <label class="form-control d-flex align-items-center justify-content-between">
                                <label for="" class="fs-14 lh-1 d-block cursor-pointer text-dark">{{ translate('Status') }}</label>
                                <label class="switcher cmn_focus rounded-pill">
                                    <input class="switcher_input update-business-setting" id="driverSelfRegistration"
                                           name="driver_self_registration" type="checkbox" data-name="driver_self_registration"
                ...
                <div class="col-lg-6">
                    <div class="card card-body h-100">
                        ...
                        <div class="p-lg-4 p-3 rounded bg-F6F6F6">
                            ...
                            <label class="form-control d-flex align-items-center justify-content-between">
                                <label for="driverVerification" class="fs-14 lh-1 d-block cursor-pointer text-dark">{{ translate('Status') }}</label>
                                <label class="switcher cmn_focus rounded-pill">
                ...
                <div class="col-lg-6">
                    <div class="card card-body h-100">
                        ...
                        <div class="p-lg-4 p-3 rounded bg-F6F6F6">
                            ...
                            <label class="form-control d-flex align-items-center justify-content-between">
                                <label for="driverReview" class="fs-14 lh-1 d-block cursor-pointer text-dark">{{ translate('Status') }}</label>
                                <label class="switcher cmn_focus rounded-pill">
                    ...
                    <div class="card card-body h-100">
                        <div class="mb-20">
                            <h5 class="d-flex align-items-center gap-2 justify-content-between text-capitalize mb-2">
                                ...
                                <a href="{{ route('admin.driver.level.index') }}" class="text-link fs-12 fw-semibold d-flex gap-1 align-items-center">
                                   ...
                                   <div>
                                       <label for="equivalent_points"
                                        ...
                                        <input type="tel" name="loyalty_points[value]" id="equivalent_points"
                                            ...
                                            value="{{ $settings->where('key_name', 'loyalty_points')->first()?->value['points'] }}" tabindex="6">
                                   </div>
                               </div>
                           </div>
                           <div class="btn--container justify-content-end mt-4">
                               <button type="reset" class="btn btn-secondary min-w-120 cmn_focus" tabindex="7">{{ translate('reset') }}</button>
                               <button type="submit" class="btn btn-primary min-w-120 cmn_focus" tabindex="8">{{ translate('save') }}</button>
                           </div>
                       </form>
                    </div>
                </div>
                        ...
                        <form action="{{ route('admin.business.setup.driver.store') }}?type=maximum_parcel_request_accept_limit" id="maximumParcelRequestAcceptLimit" method="post">
                           ...
                           <div class="p-lg-4 p-3 rounded bg-F6F6F6">
                               <div class="mb-20">
                                   ...
                                   <label class="form-control d-flex align-items-center justify-content-between">
                                       <label for="maximum_parcel_request_accept_limit" class="fs-14 lh-1 d-block cursor-pointer text-dark">{{ translate('Status') }}</label>
                           ...
                           <div class="btn--container justify-content-end mt-4">
                               <button type="reset" class="btn btn-secondary min-w-120 cmn_focus" tabindex="11">{{ translate('reset') }}</button>
                               <button type="submit" class="btn btn-primary min-w-120 cmn_focus" tabindex="12">{{ translate('save') }}</button>
                           </div>
                       </form>
                    </div>
                </div>
                <div class="col-12">
                    <div class="card card-body h-100 collapsible-card-body">
                        ...
                        <div class="collapsible-card-content mt-4">
                            <form action="{{ route('admin.business.setup.driver.vehicle-update') . '?type=' . DRIVER_SETTINGS }}"
                                        ...
                                        @foreach(UPDATE_VEHICLE as $updateVehicle)
                                            <label class="custom-checkbox rounded">
                                                <input type="checkbox" class="module-checkbox text-capitalize"
                                                        name="update_vehicle[]"
                                                        value="{{$updateVehicle}}" tabindex="14" {{in_array($updateVehicle, $settings->firstWhere('key_name', 'update_vehicle')?->value ??[], true) ? "checked" : ""}}>
                               ...
                               <div class="btn--container justify-content-end mt-4">
                                   <button type="reset" class="btn btn-secondary min-w-120 cmn_focus" tabindex="15">{{ translate('reset') }}</button>
                                   <button type="submit" class="btn btn-primary min-w-120 cmn_focus" tabindex="16">{{ translate('save') }}</button>
                               </div>
                           </form>
                        </div>
                    </div>
                ...
                <div class="col-12">
                    ...
                    <div class="card card-body h-100 collapsible-card-body">
                        <div class="d-flex gap-2 justify-content-between align-items-center">
                            ...
                            <div class="d-flex gap-2 align-items-center">
                                <a href="javascript:" class="text-link fw-semibold fs-12 text-nowrap d-flex view-btn">
                                    <span>{{ translate('View') }}</span> <span><i class="{{ ($settings->firstWhere('key_name', 'cash_in_hand_setup_status')?->value ?? 0) == 1 ? 'tio-arrow-downward' : 'tio-arrow-upward' }}"></i></span>
                                </a>
                        ...
                        <div class="collapsible-card-content mt-4">
                            <form action="{{ route('admin.business.setup.driver.update-cash-in-hand-setup')}}" method="POST">
                                @csrf
                               <div class="p-lg-4 p-3 rounded bg-F6F6F6">
                                   <div class="row g-lg-4 g-3">
                                        <div class="col-md-6">
                                            <div class="">
                                                <label for="" class="mb-2">
                                                    {{ translate('Max_Amount_to_Hold_Cash') }} ({{ ((session()->get('currency_symbol') ?? businessConfig('currency_symbol', 'business_information')?->value) ?? "$") }})
                                        ...
                                        <div class="col-md-6">
                                            <div class="">
                                                <label for="" class="mb-2">
                                                    {{ translate('Minimum Amount to Pay') }} ({{ ((session()->get('currency_symbol') ?? businessConfig('currency_symbol', 'business_information')?->value) ?? "$")}})</label>
                                                    <span class="text-danger">*</span>
                                                <input type="text"
                                                       value="{{ $settings->firstWhere('key_name', 'min_amount_to_pay')?->value ?? 20 }}" class="form-control"
                               ...
                               <div class="btn--container justify-content-end mt-4">
                                   <button type="reset" class="btn btn-secondary min-w-120 cmn_focus" tabindex="20">{{ translate('reset') }}</button>
                                   <button type="submit" class="btn btn-primary min-w-120 cmn_focus" tabindex="21">{{ translate('save') }}</button>
                               </div>
                           </form>
                        </div>
                    </div>
