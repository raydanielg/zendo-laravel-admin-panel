...
class TripRequestController extends Controller
{
    ...
    public function requestAction(Request $request): JsonResponse
    {
        ...
        $maximumAmountToHoldCash = businessConfig('max_amount_to_hold_cash')?->value && businessConfig('cash_in_hand_setup_status')?->value ? businessConfig('max_amount_to_hold_cash')?->value : null;
        $payableBalance = $user?->userAccount->payable_balance > $user?->userAccount->receivable_balance ? ($user?->userAccount->payable_balance - $user?->userAccount->receivable_balance) : 0;
        if ($maximumAmountToHoldCash && $payableBalance >= $maximumAmountToHoldCash)
        {
            return response()->json(responseFormatter(MAXIMUM_AMOUNT_TO_HOLD_CASH_EXCEEDS), 403);
        }
